...
For FX header controls, put `// @control(_Parameter)` immediately after the `// @whimtex-effect Category/Name` marker, or first when no marker exists. It binds one existing, singly declared bool, enum, float, color or float2/3/4 parameter to a compact field before the FX menu. `hidden` suppresses its body row only; otherwise both fields edit the same value. Multiple directives warn and the last wins. Unsupported types show no header field. This UI directive adds no shader behavior or automatic opacity blending. See [FX block control](../ShaderFX.md#fx-block-control).
