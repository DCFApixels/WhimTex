...
修复会同时匹配颜色和纹理，保留细节而不是将其抹平；无法重建周围图像中不存在的独特细节。
相容的平滑片段会混合以减轻拼接痕迹；锐利细节保留原始供体样本。
对平滑噪声进行大范围修复时，仍可能丢失对比度并留下较平坦的条带。
存在匹配质量相近的替代样本时，会对过度使用的来源区域施加软惩罚。这能减少重复，但不会完全禁止重复。
**Hardness** 控制笔触边缘的混合，不控制重建细节的锐度。
重建先从已知边缘向内推进，再整体优化，有助于延续跨越缺口的线条；
但不明确的大块形状仍可能需要手动修整。
...
沿四边绘制时，搜索会将相对边缘一起处理。复杂或不明确的纹理仍可能出现片段间的过渡。
