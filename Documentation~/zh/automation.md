...
智能体也能修改现有 FX 参数、查找已安装的预设、重排或复制效果，并将效果烘焙为 Drawing
像素。它可以复制、删除、合并或转换图层，执行 Blur 和 Healing 笔画，并查看效果前后的
图像。打开窗口中的即时编辑合并为一个 Undo 步骤，不会自动保存。栅格化程序化图层或组
会丢失其程序化内容；如需以后继续调整，请保留副本。
