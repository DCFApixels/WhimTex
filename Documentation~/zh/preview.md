...
工具设置栏始终保留一行空间，即使工具没有设置项。面板较窄时，设置会换行并覆盖在预览上方，不会移动预览或改变其大小。用于拖出参考线的边条始终位于设置下方，保持可用。

工具栏分隔线下方的工具会根据当前上下文出现。选中 Gradient 图层时，自动启用手形图标的 **Gradient Handles**。手动切回基本工具后，该选择会保持到活动图层改变。多选时，上下文由最后选中的活动图层决定。

启用 **UV** 会添加 **UV Island Select**，但不会自动切换工具。FX 参数的 **Edit on Canvas** 会在工具栏最下方添加一个临时手形工具。按钮提示和预览页脚标明正在编辑的参数；编辑其他参数会替换此临时工具。

再次点击当前上下文工具或按 `Escape`，会返回上次使用的基本工具。退出临时 FX 工具时，会返回进入前的工具，包括仍可用的上下文工具。拖动时，`Escape` 只取消当前拖动；再次按下才退出工具。操作 Inspector、打开 Color Picker 或切换应用不会退出工具。
