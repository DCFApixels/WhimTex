// @whimtex-effect Stylization/Posterize
// @control(_Opacity)
// @param hidden float _Opacity = 1 [0 .. 1] // Blend between the original colors and the posterized result.
// @group(Tonal Steps)
...
// @endgroup
// @group(Dithering; _Dither)
// @param hidden enum _Dither = None {None: 0, Bayer2: 1, Bayer4: 2, Bayer8: 3, Interleaved: 4, Checker: 5, Halftone: 6, Hash: 7} // Dither pattern, applied per pixel; the same list as in Pixelate.
// @if _Dither != 0
...
float4 ApplyFX(float2 uv, float4 color)
{
	...
	float4 result = color;
	result.rgb = pow(min(floor(c * steps + threshold), steps) / steps, 1.0 / gamma);
