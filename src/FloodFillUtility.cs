...
{
    internal static class FloodFillUtility
    {
        internal static bool Fill(
            NativeArray<Color32> source, NativeArray<Color32> reference, NativeArray<byte> valid,
            NativeArray<Color32> output, int width, int height, int seed, Color32 color,
            int tolerance, int expand, bool antialias, bool contiguous = true)
        {
            ...
            new ColorJob
            {
                ...
                color = color, expand = Mathf.Clamp(expand, 0, 32), antialias = antialias
            }.Run();
            return changed[0] != 0;
        }
        ...
        {
            [ReadOnly] public NativeArray<Color32> reference;
            ...
            private void Visit(int index)
            {
                ...
                Color32 sample = reference[index];
                ...
                int limit = tolerance * 255;
                if (Mathf.Abs(sample.a - target.a) > tolerance ||
        ...
        private struct ColorJob : IJob
        {
            ...
            public void Execute()
            {
                ...
                {
                    Color32 before = source[i];
                    output[i] = before;
                    if (valid[i] == 0) continue;
                    float coverage = antialias ? Mathf.Clamp01(expand + 1.5f - distance[i]) : distance[i] <= expand ? 1f : 0f;
                    float alpha = color.a / 255f * coverage;
                    if (alpha <= 0f) continue;
                    float retained = before.a / 255f * (1f - alpha);
                    float resultAlpha = alpha + retained;
                    Color32 after = new Color32(
                        Byte((color.r * alpha + before.r * retained) / resultAlpha),
                        Byte((color.g * alpha + before.g * retained) / resultAlpha),
                        Byte((color.b * alpha + before.b * retained) / resultAlpha), Byte(resultAlpha * 255f));
                    output[i] = after;
            ...
            private static byte Byte(float value) => (byte)Mathf.Clamp(Mathf.RoundToInt(value), 0, 255);
        }
        ...
        {
            [ReadOnly] public NativeArray<Color32> source;
            [WriteOnly] public NativeArray<Color32> reference;
            [WriteOnly] public NativeArray<byte> valid;

            public void Execute(int index)
            {
                reference[index] = source[index];
                valid[index] = 1;
            }
        }
        ...
        {
            [ReadOnly] public NativeArray<Color32> composite;
            [WriteOnly] public NativeArray<Color32> reference;
            [WriteOnly] public NativeArray<byte> valid;
