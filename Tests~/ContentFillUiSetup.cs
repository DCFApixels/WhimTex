...
doc.layers.Add(new DCFApixels.SpriteEditor.Layer(new DCFApixels.SpriteEditor.ColorFillLayerBehaviour{color=new UnityEngine.Color(.125f,.5f,.25f,1)}));
...
selection.GetType().GetMethod("Set",f).Invoke(selection,new object[]{mask,System.Enum.Parse(type.Assembly.GetType("DCFApixels.SpriteEditor.SelectionCombine"),"Replace")});
