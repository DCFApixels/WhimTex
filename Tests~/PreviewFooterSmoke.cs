...
try
{
    foreach(int width in new[]{900,600,599,400,280,200})
    {
        ...
        Check(updates.childCount==2&&updates[0].ClassListContains("whimtex-preview-quality")&&updates[1].ClassListContains("whimtex-live-output-button"),"Quality and Live Update left");
        ...
        Check(footer.ClassListContains("whimtex-preview-footer--compact")== (width<600),"Compact breakpoint");
        ...
        foreach(string text in new[]{"Add a layer to start",new string('W',300),"", "Ready"})
        {
            ...
            Check(hint.ClassListContains("whimtex-preview-status--hidden") == (text.Length==0||!(required<=hint.contentRect.width)),"Hide the entire hint only when empty or too wide");
        }
