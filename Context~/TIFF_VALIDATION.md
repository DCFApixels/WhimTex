# История проверок TIFF

2026-09-19; Unity **6000.7.0a6**, Windows Editor / Windows64 Mono Player, DX12.
Это исторический отчёт с условиями и результатами отдельных прогонов, а не текущий статус формата
или подтверждение готовности всех платформ. TIFF — основной формат документов с версии 0.11.0;
`.asset` остался только для чтения и миграции, агентские команды работают с TIFF.
Текущий контракт: [DOCUMENT_FORMAT.md](DOCUMENT_FORMAT.md).
...
Исторические приоритеты по результатам baseline-замера (не список оставшихся задач).
Потоковая запись полос, кеш блоков с SHA-256 и ленивое чтение Drawing уже реализованы;
актуальные ограничения описаны в [DOCUMENT_FORMAT.md](DOCUMENT_FORMAT.md).
...
4. На момент baseline-замера Open восстанавливал все Drawing-пиксели; требовался отдельный lifecycle/Undo-дизайн ленивого чтения.
